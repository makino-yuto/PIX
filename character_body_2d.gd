...
func _physics_process(delta):
	...
	if abs(position.y - Global.player_position.y) > 1000:
		teleport_to_spawn()
	...
	if is_on_floor() and is_on_wall():
		velocity.y = -120
	elif Input.is_action_just_pressed("jump") and is_on_floor() and abs(position.x - Global.player_position.x) < 50 and abs(position.y - Global.player_position.y) < 50:
		velocity.y = -200
